...
// 所有中文文本
const T = {
  template: '模板', editor: '编辑器', settings: '设置',
  createTpl: '新建模板', journalPlaceholder: '输入期刊名或缩写，如 iotj / ieee / cvpr',
  chooseFile: '选择文件', noFile: '未选择', formatLatex: 'LaTeX', formatDocx: 'Word',
  create: '创建', savedTpls: '模板库', noTpls: '还没有模板。在上方搜索或选择本地文件。',
  searchHint: '支持缩写：iotj → IEEE IoT, eswa → Expert Systems, cvpr → CVPR',
  searchBtn: '🔍 搜索并下载', searching: '搜索下载中，请稍候...',
  searchTimeout: '文件较大，可能需要 1-2 分钟，请耐心等待',
  searchDone: '已下载到', parsed: '已解析', parse: '解析结构', delete: '删除',
  noMatches: '未匹配到预设模板，输入完整期刊名将通过 AI 自动解析',
  noParsed: '请先选择或搜索模板，点击「解析结构」',
  modules: '模块', enterText: '输入{label}...',
  addFigure: '+ 图片', addTable: '+ 表格',
  figureCaption: '图注', figureLabel: '标签', figurePath: '图片路径', wide: '跨栏',
  previewLabel: '实时预览', previewEmpty: '在左侧填写内容，这里会实时显示',
  lineCount: '行', exportPDF: '📄 导出 PDF', exportTEX: '📥 下载 .tex',
  copySource: '📋 复制', copyOk: '已复制到剪贴板',
  createProfile: '创建用户档案',
  profileHint: '配置 AI 模型后，搜索缩写会自动解析（如 iotj → IEEE IoT Journal）',
  ...
  downloadPathLabel: '模板下载路径', downloadPathHint: '搜索下载的模板保存到此目录',
  dirChoose: '选择文件夹', backendOk: '✓ 后端已连接', tplCreated: '模板已创建',
  tplDeleted: '已删除', parsedMsg: '解析完成！发现 {count} 个模块',
  userCreated: '用户已创建', providerAdded: '模型已添加', providerRemoved: '已移除',
  fillRequired: '请填写期刊名称', confirmDelete: '确认删除？',
  searchStarted: '正在搜索模板...', searchFailed: '搜索失败，请检查网络或手动选择文件',
  downloadFailed: '下载失败，可能是网络问题，请手动选择文件',
  agentStatus: '状态', zipExtracting: '正在解压压缩包...',
  zipReady: '解压完成', zipError: '解压失败',
  zipHint: '检测到 ZIP 文件，正在自动解压',
  noFileError: '请先选择文件',
}
...
const flash = (text, type='success') => { message.value={text,type}; setTimeout(()=>message.value={text:'',type:''}, 4000) }
...
async function parseTemplate(tmpl) {
  ...
  try {
    ...
    for (const m of parsedMapping.value.modules) { if (!(m.id in userContent)&&m.type!=='figure'&&m.type!=='table') userContent[m.id]='' }
    flash(t('parsedMsg',{count:parsedMapping.value.modules.length})); activeTab.value='editor'; agentStatus.value={text:'',type:''}; triggerPreview()
  } catch(e) {
    flash(e.message,'error')
    agentStatus.value={text:'解析失败，尝试用 AI 兜底...',type:'warning'}
    try {
      const r = await api('/pipeline/run',{method:'POST',body:JSON.stringify({journal_name:tmpl.journal_name,template_format:tmpl.template_format,template_path:tmpl.download_path,user_content:userContent,user_config_id:userConfig.value?.id||null})})
      pollResult(r.task_id, 'pipeline')
...
async function generatePreview() {
  ...
  if (!Object.values(userContent).some(v=>v&&v.trim())) { previewText.value=''; return }
  try {
    const r = await api('/preview',{method:'POST',body:JSON.stringify({
    ...
    previewText.value=r.assembled_text; previewLines.value=r.line_count
  } catch {}
}
function copyPreview() { navigator.clipboard.writeText(previewText.value).then(()=>flash(t('copyOk'))) }
function downloadTex() {
  if (!previewText.value) return
  const b=new Blob([previewText.value],{type:'application/x-tex'})
  const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download='paper_assembled.tex'; a.click()
}
async function downloadPdf() {
  if (!parsedMapping.value) return flash(t('fillRequired'),'error')
  loading.value=true
  try {
    const r = await fetch(`${API}/export/pdf`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
      template_path: templateForm.download_path||parsedMapping.value.template_path,
    ...
    a.download=r.headers.get('Content-Disposition')?.match(/filename="?(.+?)"?/)?.[1]||'paper.pdf'; a.click()
  } catch(e) { flash(e.message,'error') }
  finally { loading.value=false }
}
...
function pickDirectory() {
  ...
  if ('showDirectoryPicker' in window) { window.showDirectoryPicker().then(h=>{downloadPath.value=h.name}).catch(()=>{}) }
  else { d.click() }
}
...
<div class="app-shell">
  <header class="app-header">
    ...
    <nav class="nav-tabs">
      <button :class="['nav-tab',{active:activeTab==='template'}]" @click="activeTab='template'">{{ T.template }}</button>
  ...
  <!-- ====== 模板 ====== -->
  <main v-if="activeTab==='template'" class="panel">
      ...
      <div class="form-row" style="position:relative">
        <div style="flex:3;position:relative">
          <input v-model="templateForm.journal_name" :placeholder="t('journalPlaceholder')" class="input"
            @focus="showSuggestions=suggestions.length>0" @blur="setTimeout(()=>showSuggestions=false,200)" style="width:100%" />
  ...
  <main v-else-if="activeTab==='editor'" class="panel editor-split">
    <div class="editor-left">
      <div v-if="!parsedMapping" class="empty-state"><div class="empty-icon">📝</div><p>{{ t('noParsed') }}</p><button @click="activeTab='template'" class="btn btn-primary" style="margin-top:1rem">{{ T.template }}</button></div>
